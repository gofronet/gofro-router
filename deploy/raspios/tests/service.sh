...
SERVICE="$ROOT/deploy/raspios/root/usr/libexec/gofro/service"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
GOFRO_UPDATE_LOCK="$TMP/lock" GOFRO_REBOOT_COMMAND="$TMP/systemctl" GOFRO_REBOOT_DELAY=0 GOFRO_TEST_REBOOT_LOG="$TMP/reboot.log" "$SERVICE" reboot router
[ -d "$TMP/lock" ]
while [ ! -e "$TMP/reboot.log" ]; do sleep 1; done
[ "$(cat "$TMP/reboot.log")" = reboot ]

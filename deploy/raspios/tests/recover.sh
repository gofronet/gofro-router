...
mkdir -p "$TMP/app/releases/0.4.7/etc/systemd/system" "$TMP/app/releases/0.5.0" "$TMP/etc" "$TMP/units"
printf '%s\n' old > "$TMP/app/releases/0.4.7/etc/systemd/system/gofro-agent.service"
...
GOFRO_VERSION_FILE=$TMP/etc/version \
	GOFRO_UNIT_DIR=$TMP/units \
	GOFRO_SYSTEMCTL_COMMAND=: \
...
[ "$(cat "$TMP/units/gofro-agent.service")" = old ]

...
The next signed candidate workspace version is `0.5.19`. The installed signed
v0.5.18 candidate is the immutable, pinned baseline; changes require a new
...
3. Open **Actions -> Release -> Run workflow**, select `main`, and enter the
   version without the `v` prefix (`0.5.19`). Leave **publish** unchecked
...
For v0.5.19, also record the upgrade from the pinned signed v0.5.18 baseline.
